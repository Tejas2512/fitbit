...
class Preprocessor:
    ...
    def __init__(self, file_object, logger_object):
        ...
        self.n_neighbors = self.config["KNNImputer"]["n_neighbors"]
        self.weights = self.config["KNNImputer"]["weights"]

        with open(os.path.join("configfile","params.yaml"),"r") as file:
            self.config = yaml.safe_load(file)
        self.metadata = self.config["modeltraining"]["metadata"]

        self.file_object = file_object
        self.logger_object = logger_object
        self.file = "Azurelog"
        self.azure = azure_methodes(logger_object=self.logger_object,file_object=self.file)
    ...
    def separate_label_feature(self, data, label_column_name):
        ...
        try:
            ...
            X_file = "X_{}_{}.csv".format(self.date,self.current_time)
            self.azure.upload_blob(data=self.X.to_csv(index=False,header=True),container_name=self.metadata,blob_name=X_file)

            y_file = "y_{}_{}.csv".format(self.date,self.current_time)
            self.azure.upload_blob(data=self.Y.to_csv(index=False,header=True),container_name=self.metadata,blob_name=y_file)
    ...
    def impute_missing_values(self, data):
        ...
        self.data= data
        try:
            imputer=KNNImputer(n_neighbors=self.n_neighbors, weights=self.weights,missing_values=np.nan)
