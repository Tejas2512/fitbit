...
class KMeansClustering:
    ...
    def __init__(self, file_object, logger_object):
        ...
        self.lower = self.config["kmeans"]["lower_bound"]
        self.upper = self.config["kmeans"]["upper_bound"]
        self.init = self.config["kmeans"]["init"]
        self.curve = self.config["kneelocater"]["curve"]
        self.direction = self.config["kneelocater"]["direction"]
        self.random_state = self.config["base"]["random_state"]

        self.file_object = file_object
        self.logger_object = logger_object
        # self.azurelog = "Azurelog"
        # self.azure = azure_methodes(file_object=self.azurelog,logger_object=self.logger_object)
    ...
    def elbow_plot(self,data):
        ...
        try:
            ...
            # plt.ylabel('WCSS')
            # #plt.show()
            # self.azure.upload_blob(data=)
            # plt.savefig('preprocessing_data/K-Means_Elbow.PNG') # saving the elbow plot locally
            # finding the value of the optimum cluster programmatically
            self.kn = KneeLocator(range(self.lower, self.upper), wcss, curve=self.curve, direction=self.direction)
    ...
    def create_clusters(self,data,number_of_clusters):
        ...
        self.data=data
        try:
            self.kmeans = KMeans(n_clusters=number_of_clusters, init=self.init, random_state=self.random_state)
            #self.data = self.data[~self.data.isin([np.nan, np.inf, -np.inf]).any(1)]
            self.y_kmeans=self.kmeans.fit_predict(data) #  divide data into clusters

            self.file_op = file_methods.File_Operation(self.file_object,self.logger_object)
            self.save_model = self.file_op.save_model(model=self.kmeans, filename='KMeans') # saving the KMeans model to directory
